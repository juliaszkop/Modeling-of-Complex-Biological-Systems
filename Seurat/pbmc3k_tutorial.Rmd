...
pbmc.data <- Read10X(data.dir = "/Users/juliaszkop/Downloads/filtered_gene_bc_matrices/hg19")
...
```{r}
data_dir <- "/Users/juliaszkop/Desktop/matrices_mex/hg19"

pbmc.data <- Read10X(data.dir = data_dir)

pbmc <- CreateSeuratObject(counts = pbmc.data, project = "pbmc68k", min.cells = 3, min.features = 300)
...
pbmc <- subset(pbmc, subset = nFeature_RNA > 300 & nFeature_RNA < 4000 & percent.mt < 5)
...
#So in case of 68k data i will use ElbowPlot()
...
pbmc$kmeans_clusters <- as.factor(km.res$cluster)

DimPlot(pbmc, reduction = "tsne", group.by = "kmeans_clusters", label = TRUE, pt.size = 0.5) +
  ggtitle("t-SNE Projection with 10 k-means Clusters")


```
```{r}

cluster_counts <- table(pbmc$kmeans_clusters)
pct <- round(100 * cluster_counts / sum(cluster_counts), 1)
pct
...
```{r}
#3a
df <- as.data.frame(pbmc@meta.data)

p_genes <- ggplot(df, aes(x = "", y = nFeature_RNA)) + geom_violin(color = "black") + scale_y_log10() + labs( y = "Counts", x = "Genes/Cell")

p_umi <- ggplot(df, aes(x = "", y = nCount_RNA)) + geom_violin(color = "black") + scale_y_log10() + labs(y = "Counts", x = "UMIs Counts/Cell")
...
DoHeatmap(pbmc, features = top_markers$gene, group.by = "kmeans_clusters") + 
  scale_fill_gradient2(low = "darkgray", mid = "white", high = "lightgray", midpoint = 0) +
  ggtitle("Top Marker Genes for Each of 10 Clusters")
...
```{r}
#from 3d to 3i
p1 <- FeaturePlot(pbmc, features = "CD3D", cols = c("lightgray", "coral")) + ggtitle("CD3D")
p2 <- FeaturePlot(pbmc, features = "CD8A", cols = c("lightgray", "coral")) + ggtitle("CD8A")
p3 <- FeaturePlot(pbmc, features = "NKG7", cols = c("lightgray", "coral")) + ggtitle("NKG7")
p4 <- FeaturePlot(pbmc, features = "FCER1A", cols = c("lightgray", "coral")) + ggtitle("FCER1A")
p5 <- FeaturePlot(pbmc, features = "FCGR3A", cols = c("lightgray", "coral")) + ggtitle("CD16 / FCGR3A")
p6 <- FeaturePlot(pbmc, features = "S100A8", cols = c("lightgray", "coral")) + ggtitle("S100A8")
...
```{r}
pbmc.markers <- FindAllMarkers(pbmc, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
top.markers <- pbmc.markers %>% group_by(cluster) %>% slice_max(order_by = avg_log2FC, n = 5)
print(top.markers)

```
Not sure if this is right, but i tried my best 

Cluster ID --	Markers	Cell -- Type
1	--	IL7R, CD27	--	Memory CD4+ T
2	--	USP10, AQP3	--	DC
3	--	GZMK, GZMA, NCR3	--	CD8+ T
4	--	FCN1, SERPINA1, PILRA	--	CD14+ Mono
5	--	IL7R, DUSP2, HMGB2	--	Activated T
6	--	CCR7, LEF1, ID3	--	Naive CD4+ T
7	--	MS4A1, CD79A, TCL1A	--	B
8	--	GNLY, GZMB, FGFBP2	--	NK
9	--	CCR7, LEF1, HIST1H4C	--	Naive CD4+ T
10 --	CCR7, LEF1, SELL	--	Naive CD4+ T

```{r}
new.cluster.ids <- c("Memory CD4+ T", "DC", "CD8+ T", "CD14+ Mono", "Activated T", "Naive CD4+ T", "B", "NK", "Naive CD4+ T", "Naive CD4+ T")
names(new.cluster.ids) <- levels(pbmc)
pbmc <- RenameIdents(pbmc, new.cluster.ids)
