...
png("szkop_problem1.png",height=700,width=700)
heatmap.2(edata,
          main = " Problem 1 - Bottomly et al. Heatmap", 
          notecol="black",     
          density.info="none",  
          trace="none",        
          margins =c(12,9),    
...
plot(svd.out$u[,1], svd.out$u[,2], pch=20, xlab="Left Singular Vector 1", ylab="Left Singular Vector 2")

dev.copy(pdf, "szkop_problem3.pdf")
dev.off()
...
df <- data.table(svd.out$u[,1:5])

df_m <- melt(df)

ggplot(df_m, aes(x=variable, y = value)) + geom_violin(draw_quantiles = c(0.25, 0.5, 0.75))

ggsave("szkop_problem4.pdf")
...
tsne_out <- Rtsne(edata,pca=FALSE,perplexity=60)
tsne_out = data.table(tsne_out$Y,cluster=as.factor(clusters))

ggplot(tsne_out) + geom_point(aes(x=V1, y=V2, col=cluster))

ggsave("szkop_problem5.pdf")
