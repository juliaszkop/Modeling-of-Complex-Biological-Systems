...
```{r}
table_batch_cancer <- table(Batch = pheno$batch, Cancer = pheno$cancer)
print(table_batch_cancer)

```

We can see that the Biopsy category is found only in batches 4 and 5, while Normal is found only in batch 2 and 3. This distribution indicates a potential confounding between the Biopsy effect and the batch effect, as some phenotypic categories are strictly assigned to one batch. 



```{r}

df_table <- as.data.frame(table_batch_cancer)
colnames(df_table) <- c("Batch", "CancerStatus", "Count")

ggplot(df_table, aes(x=Batch, y=Count, fill=CancerStatus)) +
  geom_bar(stat="identity", position="dodge") +
  labs(title="Distribution of samples in batches vs. CancerStatus")

ggsave("hw_problem1.png")
...
```{r}
ord <- order(pheno$batch)
edata_ordered <- log2(edata)[, ord]

png("hw_problem2_heatmap_before_ComBat.png", height=700, width=700)
heatmap.2(edata_ordered,
          ...
          col = my_palette,
          margins = c(12, 9),
          labCol = pheno$batch[ord],
          main = "Bladder Cancer Data (Before ComBat)")
dev.off()

combat_edata <- ComBat(dat = log2(edata), batch = pheno$batch, mod = model.matrix(~1, data = pheno),
                         par.prior = TRUE, prior.plots = FALSE)
combat_edata_ordered <- combat_edata[, ord]

png("hw_problem2_heatmap_after_ComBat.png", height=700, width=700)
...
```{r}
edata_log <- log2(edata)
cor_mat <- cor(edata_log)

sample_labels <- paste0(pheno$cancer, "_Batch", pheno$batch)
rownames(cor_mat) <- sample_labels
colnames(cor_mat) <- sample_labels


png("hw_problem3.png", height=700, width=700)
heatmap.2(cor_mat,
...
edata_bottomly <- exprs(bottomly.eset)
pheno_bottomly <- pData(bottomly.eset)

pheno_bottomly <- pheno_bottomly[colnames(edata_bottomly), ]
stopifnot(identical(colnames(edata_bottomly), rownames(pheno_bottomly)))

pheno_bottomly$strain <- factor(pheno_bottomly$strain)
pheno_bottomly$experiment.number <- factor(pheno_bottomly$experiment.number)

```

```{r}

mod_uncorrected <- lm(t(edata_bottomly) ~ strain + experiment.number, data = pheno_bottomly)

mod_uncorrected_tidy <- tidy(mod_uncorrected)

coef_uncorrected <- mod_uncorrected_tidy %>% filter(term == "strainDBA/2J")

```

```{r}

ggplot(coef_uncorrected, aes(x = estimate)) +
  geom_histogram(bins = 100, fill = "darkorange") +
  xlab("Coefficient Estimate for strain DBA/2J") +
  ggtitle("Distribution of DBA/2J Coefficients (Uncorrected)")
ggsave("hw_problem4_distribution_of_DBA2J_Coefficients_Uncorrected.png")

ggplot(coef_uncorrected, aes(x = p.value)) +
  geom_histogram(bins = 100, fill = "darkorange") +
  xlab("P-values for strain DBA/2J") +
  ggtitle("Distribution of P-values (Uncorrected)")
ggsave("hw_problem4_distribution_of_P-values_Uncorrected.png")
```
```{r}
mod_matrix <- model.matrix(~ strain, data = pheno_bottomly)

combat_edata_bottomly <- ComBat(dat = edata_bottomly, batch = pheno_bottomly$experiment.number, mod = mod_matrix, par.prior = TRUE, prior.plots = FALSE)
```
```{r}
mod_corrected <- lm(t(combat_edata_bottomly) ~ strain, data = pheno_bottomly)
mod_corrected_tidy <- tidy(mod_corrected)

coef_corrected <- mod_corrected_tidy %>% filter(term == "strainDBA/2J")
```

```{r}

ggplot(coef_corrected, aes(x = estimate)) +
  geom_histogram(bins = 100, fill = "darkorange") +
  xlab("Coefficient Estimate for strain DBA/2J (Corrected)") +
  ggtitle("Distribution of DBA/2J Coefficients (Corrected)")
ggsave("hw_problem4_distribution_of_DBA2J_Coefficients_corrected.png")

ggplot(coef_corrected, aes(x = p.value)) +
  geom_histogram(bins = 100, fill = "darkorange") +
  xlab("P-values for strain DBA/2J") +
  ggtitle("Distribution of P-values (Corrected)")
ggsave("hw_problem4_distribution_of_pvalue_corrected.png")
```

```{r}

est_compare <- tibble(
  LinearModel =  mod_uncorrected_tidy %>%
  filter(term == "strainDBA/2J") %>% select("estimate") %>% unlist,
  ComBat = mod_corrected_tidy %>%
  filter(term == "strainDBA/2J") %>% select("estimate") %>% unlist)

ggplot(est_compare, aes(x=LinearModel, y=ComBat)) +
     geom_point(col="darkgrey", alpha=.5, size=.5) + geom_abline(intercept=0, slope=1, col="darkred") + geom_smooth(method = "lm", se = TRUE)  + theme_bw()
ggsave("hw_problem4.png")
```
...
```{r}
mod_full <- model.matrix(~as.factor(pheno_bottomly$strain), data = pheno_bottomly)
mod_null <- model.matrix(~1, data = pheno_bottomly)

set.seed(1)
rnorm(1)

edata_bottomly_clean <- edata_bottomly[apply(edata_bottomly, 1, var) > 0, ]
n_sv <- num.sv(edata_bottomly_clean, mod_full, method = "be")
sva_out <- sva(edata_bottomly_clean, mod_full, mod_null, n.sv = n_sv)
...
mod_sva <- lm(t(edata_bottomly) ~ as.factor(pheno_bottomly$strain) + sva_out$sv)
...
coef_combat <- mod_corrected_tidy %>% filter(grepl("strain", term)) %>% pull(estimate)
coef_sva <- mod_sva_tidy %>% filter(grepl("strain", term)) %>% pull(estimate)
...
ggplot(est_compare, aes(x = ComBat, y = SVA)) +
  geom_point(alpha = 0.5, size = 1, color = "darkgrey") +
  geom_abline(intercept = 0, slope = 1, col = "darkred") +
  geom_smooth(method = "lm", se = TRUE) +
  theme_bw() +
  ggtitle("ComBat vs. SVA")
ggsave("hw_problem5.png")
```

```{r}

p_combat <- mod_corrected_tidy %>% 
  filter(grepl("strain", term, ignore.case = TRUE)) %>% 
  pull(p.value)
...
ggplot(pvalues_compare, aes(x = ComBat)) +
  geom_histogram(bins = 100, fill = "darkorange") +
  ggtitle("Histogram p-value: ComBat")
ggsave("hw_problem5_histogram_combat.png")
ggplot(pvalues_compare, aes(x = SVA)) +
  geom_histogram(bins = 100, fill = "darkorange") +
  ggtitle("Histogram p-value: SVA")
ggsave("hw_problem5_histogram_sva.png")
```